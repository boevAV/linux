...
void handle_message(string message, int sock, Client sender) {
    ...
    if (!sender.get_name().empty()) {  // Check if the sender name is set
        int existing_client_sock = find_client(sender.get_name());
        if (existing_client_sock != -1 && existing_client_sock != sock) {
            const char* response = "Name already in use. Please choose another name.\n";
