...
#define DAEMON_NAME "wdird"
#define WORKING_DIRECTORY  "/"
#define CFG_PATH "/home/nastya/test_for_d/daemon.ini"
...
void daemonize() {
    pid_t pid, sid;
    //Ответлвяемся от родительского процесса
    pid = fork();
    ...
    //Если с PID'ом всё получилось, то родительский процесс можно завершить
    if (pid > 0) {
        exit(EXIT_SUCCESS);
    }
    ...
    //Создание нового SID для дочернего процесса - иначе дочерний процесс станет сиротой
    sid = setsid();
    ...
    //Изменяем текуший рабочий каталог на место, гарантировано присутствующее в системе (/)
    ...
    //Закрываем стандартные файловые дескрипторы - излишни и создают угрозу безопасности
    close(STDIN_FILENO);
    close(STDOUT_FILENO);
    close(STDERR_FILENO);
}
...
void signal_handler(int sig) {
    switch(sig) {
        case SIGHUP:
            syslog(LOG_INFO, "Changing period or directory");
            struct Cfg cfg;
...
void task(const char* directory) {
    ...
    if (stat(directory, &dir_info) != 0) { // Get file status information
        return;
    }

    //st_mode - режим файда
    //st_mtime - время последней записи в файл 
    if (S_ISREG(dir_info.st_mode)) {
        time_t access_time = dir_info.st_atime; 

        time_t current_time;
        time(&current_time);

        if ((int)current_time - global_cfg.period < (int)access_time) {
            struct tm *tm_info = localtime(&access_time);
            char time_buffer[20];
            strftime(time_buffer, 20, "%Y-%m-%d %H:%M:%S", tm_info);
            syslog(LOG_INFO, "%s %s %s %s", "File:", directory, "was accessed at", time_buffer);
        }
...
int main() {
    ...
    signal(SIGHUP, signal_handler); // Catch handup signal
    signal(SIGTERM, signal_handler);

    struct Cfg cfg;
    if (read_config(CFG_PATH, &cfg)) { // Catch kill signal
